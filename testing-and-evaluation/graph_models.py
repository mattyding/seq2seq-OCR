...
-------------------------
Graphs the models used in the paper.

NOTE: Several graphs are rendered with matplotlib's LaTeX rendering. For instructions 
on how to install LaTeX rendering, see: https://matplotlib.org/stable/tutorials/text/usetex.html
"""
...
from settings_testing import FIGURE_DIRECTORY, RECOG_EVAL_DIRECTORY

def main():
   graph("accuracy-scatter", "basic")
   graph("accuracy-scatter", "size-color")
   graph("accuracy-regression", "basic")
   graph("accuracy-regression", "size-color")
...
def graph(graphType: str, graphLevel: str) -> None:
   """
   ...
   if (graphLevel == "size-color"):
      ...
      large_files = SizeBucket(1000, 2500, "blue")
   ...
   # Linear regressions of pred accuracy as a function of orig accuracy
   elif (graphType == "accuracy-regression"):
      plt.plot([-100, 200], [-100, 200], color="#000000", linestyle="dashed", linewidth="0.5")
      plt.xlabel("Original File Readability (%)")
      plt.ylabel("Predicted File Readability (%)")
      ...
      if (graphLevel == "basic"):
         ...
         plt.plot([], c="black", marker="s", markersize=10, label="y={:.2f}x+{:.2f}".format(m, b))
         legend = plt.legend(loc="lower right", fancybox=True)
      
      elif (graphLevel == "size-color"):
         for bucket in ordered_buckets:
            m, b = np.polyfit(*zip(*bucket.contents), 1)
            plt.plot(np.arange(0, 100, 0.1), m * np.arange(0, 100, 0.1) + b, color=bucket.color)
...
def checkParams(graphType, graphLevel):
   ...
   """
   possTypes = ["accuracy-scatter", "accuracy-regression", "flat-change"]
